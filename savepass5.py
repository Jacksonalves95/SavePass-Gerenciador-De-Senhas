...
# ─────────────────────────────────────────────
#  CITYSCAPE ANIMATION (Login Background)
# ─────────────────────────────────────────────
...
class LoginScreen(tk.Frame):
    """
    Tela de login. Layout: painel esquerdo (form) +
    canvas direito (cidade animada).
    O canvas e criado como filho direto do root,
    nao do LoginScreen, para evitar problemas de
    path no Tcl/Tk do Windows.
    """
    PANEL_BG = "#07090E"
    CITY_BG  = "#06080F"
    PANEL_W  = 480
    ...
    BUILDING_PALETTES = [
        ("#0F1520", "#F5E642", "#2A3510"),
        ("#0C1018", "#38BDF8", "#0F2030"),
        ("#100A0A", "#F87171", "#2A0F0F"),
        ("#0A1010", "#34D399", "#0A2018"),
        ("#0F0F18", "#A78BFA", "#180F2A"),
        ("#0E1015", "#FB923C", "#261408"),
    ]

    def __init__(self, parent, on_login, on_register):
        super().__init__(parent, bg=self.CITY_BG)
        ...
        panel = tk.Frame(self, bg=self.PANEL_BG,
        ...
        # ── Canvas da cidade (direita) ────────────────────────────────────────
        self._cv = tk.Canvas(self, bg=self.CITY_BG, highlightthickness=0)
        self._cv.pack(side="left", fill="both", expand=True)
        self._cv.bind("<Configure>", self._on_cv_resize)

        # ── Logo ──────────────────────────────────────────────────────────────
        logo_frame = tk.Frame(panel, bg=self.PANEL_BG)
        logo_frame.pack(pady=(50, 0), padx=40, anchor="w")
        try:
            from PIL import Image, ImageTk
            img = Image.open(LOGO_FILE)
            img.thumbnail((220, 100), Image.LANCZOS)
            self._logo_img = ImageTk.PhotoImage(img)
            tk.Label(logo_frame, image=self._logo_img,
                     bg=self.PANEL_BG).pack(anchor="w")
        except Exception:
            row = tk.Frame(logo_frame, bg=self.PANEL_BG)
            row.pack(anchor="w")
            tk.Label(row, text="Save", bg=self.PANEL_BG,
                     fg=C["white"], font=FONT_LOGO).pack(side="left")
            tk.Label(row, text="Pass", bg=self.PANEL_BG,
                     fg=C["accent"], font=FONT_LOGO).pack(side="left")

        tk.Label(panel, text="Seu cofre de senhas pessoal.",
                 bg=self.PANEL_BG, fg=C["text3"],
                 font=FONT_SMALL).pack(padx=40, anchor="w", pady=(4, 0))

        # ── Separador ─────────────────────────────────────────────────────────
        tk.Frame(panel, height=1, bg=C["border"]).pack(
            fill="x", padx=40, pady=(30, 0))

        # ── Titulo ────────────────────────────────────────────────────────────
        tk.Label(panel, text="Bem-vindo de volta",
                 bg=self.PANEL_BG, fg=C["text"],
                 font=FONT_HEADER).pack(padx=40, pady=(24, 2), anchor="w")
        tk.Label(panel, text="Entre com suas credenciais para continuar",
                 bg=self.PANEL_BG, fg=C["text3"],
                 font=FONT_SMALL).pack(padx=40, anchor="w", pady=(0, 20))

        # ── Campos ────────────────────────────────────────────────────────────
        ff = tk.Frame(panel, bg=self.PANEL_BG)
        ff.pack(padx=40, fill="x")

        tk.Label(ff, text="Usuário", bg=self.PANEL_BG,
                 fg=C["text2"], font=FONT_SMALL).pack(anchor="w")
        self.user_entry = StyledEntry(ff, placeholder="Seu nome de usuário",
                                      width=300)
        self.user_entry.pack(fill="x", pady=(4, 12))

        tk.Label(ff, text="Senha", bg=self.PANEL_BG,
                 fg=C["text2"], font=FONT_SMALL).pack(anchor="w")
        pwd_row = tk.Frame(ff, bg=self.PANEL_BG)
        pwd_row.pack(fill="x", pady=(4, 0))
        self.pwd_entry = StyledEntry(pwd_row, placeholder="Sua senha",
                                     show="•", width=260)
        self.pwd_entry.pack(side="left", fill="x", expand=True)
        self.eye_btn = tk.Label(pwd_row, text="👁",
                                bg=self.PANEL_BG,
        ...
        bf = tk.Frame(panel, bg=self.PANEL_BG)
        bf.pack(pady=20, padx=40, fill="x")
        PrimaryButton(bf, "Entrar →", self._do_login,
                      width=300).pack(fill="x")
        lnk = tk.Label(bf, text="Criar nova conta",
                       bg=self.PANEL_BG, fg=C["accent"],
                       font=FONT_SMALL, cursor="hand2")
        lnk.pack(pady=(12, 0), anchor="w")
        lnk.bind("<Button-1>", lambda e: self.on_register())
        lnk.bind("<Enter>",    lambda e: lnk.config(fg=C["accent2"]))
        lnk.bind("<Leave>",    lambda e: lnk.config(fg=C["accent"]))
        ...
        self._footer_lbl = tk.Label(
            self._cv,
            text="© 2026 Jackson Alves  •  SavePass",
            bg=self.CITY_BG, fg=C["text3"], font=FONT_MICRO)

    # ── Resize do canvas ──────────────────────────────────────────────────────
    def _on_cv_resize(self, e):
        self._cw = e.width
        self._ch = e.height
        if not self._scene_built and e.width > 10 and e.height > 10:
            self._scene_built = True
            self._build_scene(e.width, e.height)
            if not self._running:
                self._running = True
                self._tick()
        # Reposiciona footer
        try:
            self._footer_lbl.place(x=e.width // 2,
                                   y=e.height - 14,
                                   anchor="center")
        except Exception:
            pass
    ...
    def _build_scene(self, w, h):
        self._stars = [
            (random.randint(0, w), random.randint(0, int(h * 0.45)),
             random.random())
            for _ in range(80)
        ]
        self._buildings = []
        x = 0
        h_min = max(80,  int(h * 0.30))
        h_max = max(h_min + 80, int(h * 0.72))
        while x < w * 3:
            pal = random.choice(self.BUILDING_PALETTES)
            bw  = random.randint(55, 130)
            bh  = random.randint(h_min, h_max)
            gap = random.randint(0, 8)
            wins = self._make_windows(bw, bh, pal)
            self._buildings.append({"x": x, "w": bw, "h": bh,
                                    "pal": pal, "wins": wins})
            x += bw + gap
        self._cycle = x

    def _make_windows(self, bw, bh, pal):
        wins = []
        ww, wh   = random.randint(7, 14), random.randint(8, 16)
        cg, rg   = random.randint(6, 12), random.randint(10, 20)
        mx, my   = random.randint(6, 14), random.randint(10, 20)
        cols = max(1, (bw - 2*mx) // (ww + cg))
        rows = max(1, (bh - 2*my) // (wh + rg))
        for r in range(rows):
            for c in range(cols):
                wins.append({
                    "dx": mx + c*(ww+cg), "dy": my + r*(wh+rg),
                    "w": ww, "h": wh,
                    "on": random.random() > 0.38,
    ...
    def _tick(self):
        if not self._running:
            return
        self._scroll = (self._scroll + self._speed) % (self._cycle / 3 or 1)
        self._draw()
    ...
    def _draw(self):
        ...
        for i in range(14):
            frac = i / 14
            r1 = int(6  + frac*10);  g1 = int(8 + frac*12);  b1 = int(15 + frac*25)
            cv.create_rectangle(0, int(frac*h*0.78),
                                w, int((frac+1/14)*h*0.78+1),
                                fill=f"#{r1:02x}{g1:02x}{b1:02x}", outline="")

        # Moon
        mx2, my2, mr = int(w*0.72), int(h*0.12), 26
        for gr, ga in [(mr+18,"#0F1020"),(mr+10,"#141828"),(mr+4,"#1A2235")]:
            cv.create_oval(mx2-gr,my2-gr,mx2+gr,my2+gr,fill=ga,outline="")
        cv.create_oval(mx2-mr,my2-mr,mx2+mr,my2+mr,fill="#F5E642",outline="")

        # Stars
        for sx, sy, ph in self._stars:
            v = int(180 + 75*(0.5 + 0.5*math.sin(t*1.2 + ph*6)))
            sz = 1 if v < 218 else 2
            cv.create_rectangle(sx,sy,sx+sz,sy+sz,
                                fill=f"#{v:02x}{v:02x}{v:02x}",outline="")

        # Buildings
        gy = int(h*0.82)
        total_w = self._cycle / 3
        for b in self._buildings:
            for rep in range(4):
                bx = b["x"] - self._scroll + rep * total_w
                if bx + b["w"] < 0 or bx > w:
                    continue
                by = gy - b["h"]
                body, won, wdim = b["pal"]
                cv.create_rectangle(bx, by, bx+b["w"], gy, fill=body, outline="")
                for wn in b["wins"]:
                    wx = bx + wn["dx"];  wy = by + wn["dy"]
                    if wn["on"]:
                        fl = 0.85 + 0.15*math.sin(t*3+wn["phase"])
                        hx = won.lstrip("#")
                        r2=min(255,int(int(hx[0:2],16)*fl))
                        g2=min(255,int(int(hx[2:4],16)*fl))
                        b2=min(255,int(int(hx[4:6],16)*fl))
                        wc = f"#{r2:02x}{g2:02x}{b2:02x}"
                    else:
                        wc = wdim
                    cv.create_rectangle(wx,wy,wx+wn["w"],wy+wn["h"],
                                        fill=wc,outline="")

        # Ground & road
        cv.create_rectangle(0,gy,w,h,fill="#080B12",outline="")
        ry = int(h*0.83);  rh = int(h*0.10)
        cv.create_rectangle(0,ry,w,ry+rh,fill="#0D1117",outline="")
        dy = ry+rh//2-2;  dw = 40;  dg = 80
        ox = int(self._scroll*2.5) % dg
        x2 = -ox
        while x2 < w+dw:
            cv.create_rectangle(x2,dy,x2+dw,dy+4,fill="#F5A623",outline="")
            x2 += dg

    # ── Util ──────────────────────────────────────────────────────────────────
    def _toggle_pwd(self, e=None):
        self._pwd_visible = not self._pwd_visible
        self.pwd_entry.entry.config(show="" if self._pwd_visible else "•")
        self.eye_btn.config(fg=C["accent"] if self._pwd_visible else C["text3"])
    ...
    def _do_login(self):
        ...
        if not user or not pwd:
            messagebox.showwarning("Atenção",
                                   "Preencha usuário e senha.", parent=self)
            return
